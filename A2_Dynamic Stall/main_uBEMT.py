...
for i in range(len(DS_models)):
    ...
    #Run simulation
    if DS_models[i] == 'Steady':
        Calc.Solver(time = time_arr, conditions = cond, DI_Model = 'Steady', DS_Model='Steady')
    elif DS_models[i] == 'DI':
        Calc.Solver(time = time_arr, conditions = cond, DI_Model = 'PP', DS_Model='Steady')
    elif DS_models[i] == 'DS':
